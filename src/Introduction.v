...
- **Instruction set semantic**
   ...
   + Control flow
      ...
      * **`Ret`** : [%NearRetDefinition] : when used after near call, normal return to the call location, also returning unspent ergs.
      * **`NearRevert`** : [%NearRevertDefinition] : when used after near call, return from near call due to a recoverable error, return unspend ergs, roll back storage/events, execute exception handler.
      ...
      * **`Ret`** : [%FarRetDefinition] : when used after far call, normal return to the call location, also returning unspent ergs.
      * **`FarRevert`** : [%FarRevertDefinition] : when used after far call, recover (from an error), return unspend ergs, roll back storage/events, execute exception handler.
