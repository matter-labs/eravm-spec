...
Definition data_page_params := {|
  ...
  Definition m := let m0 := empty data_page_params in
                let a0 := mk_int_mod_truncated 24 3%Z in
                let (a1,_) := uinc_overflow _ a0 in
                let (a2,_) := uinc_overflow _ a1 in
                let v0 := mk_int_mod_truncated 8 1%Z in
...
Program Definition mtestcase :
  option_map (List.map (fun i => int_val 8 i)) (load_multicell data_page_params (mk_int_mod_truncated 24 3%Z) 3 m) = Some [ 1%Z; 2%Z; 3%Z].
reflexivity.
Qed.
