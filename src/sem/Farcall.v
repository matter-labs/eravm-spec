...
Definition new_code_shard_id (is_call_shard:bool) shard_provided caller_shard: shard_id :=
  if is_call_shard
  then shard_provided
  else caller_shard.

Definition new_this_shard_id type is_call_shard shard_provided caller_shard : shard_id :=
  match type with
  | Delegate => caller_shard
  |_ => new_code_shard_id is_call_shard shard_provided caller_shard
  end.

Section Def.

  Context (type:farcall_type) (dest_addr:contract_address) (handler_addr: code_address)
    (call_as_static: bool) (shard_provided: bool) (abi_ptr_tag: bool).
  
(* Not implemented yet: shards *)
Inductive farcall : FarCall.params -> state -> state -> Prop :=
| farcall_fwd_fatptr: forall flags old_regs old_pages xstack0 xstack1 xstack2 new_caller_stack gs reg_context_u128 new_pages new_code_page new_mem_ctx in_ptr to_shard ergs_query ergs_actual fwd_mode is_syscall_query out_ptr,
    ...
    let new_stack := ExternalCall {|
                         ...
                         ecf_this_shard_id := new_this_shard_id type to_shard old_extframe.(ecf_this_shard_id);
                         ecf_caller_shard_id := new_caller;
                         ecf_code_shard_id := new_caller;
                         ecf_common := {|
    ...
    farcall
            ...
            {|
              ...
              gs_global       := gs;
            |}
| farcall_fwd_memory: forall gs flags old_regs old_pages xstack0 xstack1 xstack2 new_caller_stack reg_context_u128 new_pages new_code_page new_mem_ctx in_ptr to_shard ergs_query ergs_actual is_syscall_query out_ptr page_type,
    let old_extframe := topmost_extframe xstack0 in
    let current_contract := old_extframe.(ecf_this_address) in
    let is_system := addr_is_kernel dest_addr && is_syscall_query in
    let allow_masking := negb is_system in
